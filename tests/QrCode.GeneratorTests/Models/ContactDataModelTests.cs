...
public sealed class ContactDataModelTests : UnitTestBase
{
  ...
  public void ContactDataModelTest()
  {
    ...
    Assert.IsNull(model.MobilePhone);
    Assert.IsNull(model.OfficePhone);
    Assert.IsNull(model.Email);
    Assert.IsNull(model.Birthday);
    Assert.IsNull(model.WebSite);
    Assert.IsNull(model.Street);
    Assert.IsNull(model.HouseNumber);
    Assert.IsNull(model.City);
    Assert.IsNull(model.Country);
    Assert.IsNull(model.ZipCode);
    Assert.IsNull(model.Note);
    Assert.IsNull(model.StateRegion);
    Assert.AreEqual(AddressOrder.Default, model.AddressOrder);
    Assert.IsNull(model.Org);
    Assert.IsNull(model.OrgTitle);
